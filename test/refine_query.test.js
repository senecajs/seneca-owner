...
describe('refine_query', function () {
  test('read-query-single', async () => {
    ...
    // console.log(m0)
    expect(m0).to.equal({ q: { w: 1 } })

    // extends existing q
    let m1 = { q: { z: 2 } }
    refine_query(s0, m1, qp0, p0, o0, intern, {})
    // console.log(m0)
    expect(m1).to.equal({ q: { w: 1, z: 2 } })
  ...
  test('read-query-multiple', async () => {
    ...
    // injects q if needed
    refine_query(s0, m0, qp0, p0, o0, intern, {})
    expect(m0).to.equal({ q: { w: [1, 2] } })

    // extends existing q
    let m1 = { q: { z: 2 } }
    refine_query(s0, m1, qp0, p0, o0, intern, {})
    expect(m1).to.equal({ q: { w: [1, 2], z: 2 } })

    // allows valid existing q - single
    let m2 = { q: { w: 1 } }
    refine_query(s0, m2, qp0, p0, o0, intern, {})
    expect(m2).to.equal({ q: { w: 1 } })
    ...
    try {
      ...
      expect(e.code).to.equal('field-not-valid')
      partial(e.details, {
        bad_query_val: 3,
        field: 'w',
        query_val: 3,
    ...
    expect(m4).to.equal({ q: { w: [2] } })

    // allows valid existing q - multiple
    let m5 = { q: { w: [1, 2] } }
    refine_query(s0, m5, qp0, p0, o0, intern, {})
    expect(m5).to.equal({ q: { w: [1, 2] } })
    ...
    try {
      ...
      expect(e.code).to.equal('field-values-not-valid')
      partial(e.details, {
        field: 'w',
        bad_query_val: 3,
        query_val: [1, 3],
