...
function entityCovers(a: string, b: string): boolean {
  if (a === b || '*' === a) {
    return true
  }
...
function coverageRank(entity: string): number {
  if ('*' === entity) {
    return 2
  }
...
function unionCoverage(deep: any, permissions: Permission[]): Permission[] {
  return permissions.map((perm) => {
    ...
    permissions
      ...
      .forEach((other) => {
        other.ops.forEach((op: string) => ops.add(op))
...
export function build_roles(
  ...
  Object.keys(roles).forEach((name: string) => {
    ...
    const permissions = unionCoverage(
      deep,
      effectivePermissions(deep, error, roles, memo, visiting, name)
    )
